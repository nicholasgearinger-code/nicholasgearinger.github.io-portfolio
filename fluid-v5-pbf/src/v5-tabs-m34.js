// Fluid V5 M3.4.3 tab-shell loader: HDR time-of-day UI + true night-pool diagnostics.
const srcUrl=new URL('./v5-tabs.js',import.meta.url);
const response=await fetch(srcUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 tabs M3.4.3: unable to load tab shell (${response.status}).`);
let src=await response.text();
const swap=(a,b,label)=>{if(!src.includes(a))throw new Error(`Fluid V5 tabs M3.4.3: ${label} signature changed.`);src=src.replace(a,b)};
...
"  ['water','WATER','Tune the underlying water material and optical response independently of the scene mood.'],\n  ['lighting','LIGHTING','Choose Day, Sunset or Night. Each mode owns its HDR environment and exposure. Sunset uses a vivid low-sun HDR panorama; Night blacks out the sky/overhead fill so six submerged pool lights become the only meaningful illumination.'],",'tab definitions');
...
const hudReplacement="const light=window.__v5LightState;const mood=(light?.timeOfDay||'day').toUpperCase();const pool=window.__v5LightLab?.state?.poolLight?.toUpperCase?.()||'';const np=window.__v5NightPoolStatus;const es=window.__v5EnvironmentStatus;const envTag=es?.online?`${String(es.mode||'').toUpperCase()} HDRI`:`ENV ${String(es?.stage||'...').toUpperCase()}`;const nightTag=mood==='NIGHT'?` · pool ${np?.online?'6-LIGHT':'fallback'} ${pool}`:'';hud.textContent=`V5 M3.4.3 · ${q}${state?.autoQuality?' AUTO':''}\\nFPS ${fps} · SSFR ${scale}% · GPU ${pressure}%\\nPBF ${total.toLocaleString()} · fluid ${fluid.toLocaleString()} · rigid ${rigid.toLocaleString()}\\nsecondary ${m2?.secondaryCapacity?.toLocaleString?.()||'--'} · drained ${m2?.drainedTotal?.toLocaleString?.()||'0'}\\natomic ${atomic.trim()} · ${mood} · ${envTag}${nightTag} · UW ${uw.toFixed(2)} m`;";
swap(hudNeedle,hudReplacement,'compact HUD');
src=src.replace('The map is generated from live refracted sunlight and projected across the pool floor.','Day/Sunset use the atomic sun projector. Sunset also uses a dedicated HDR panorama. Night has no solar source: the HDR environment is near-black and the pool is lit by the six submerged fixtures only.');
const blobUrl=URL.createObjectURL(new Blob([src],{type:'text/javascript'}));
try{await import(blobUrl)}finally{URL.revokeObjectURL(blobUrl)}
console.info('[Fluid V5 UI] M3.4.3 HDR time-of-day tab enabled.');
