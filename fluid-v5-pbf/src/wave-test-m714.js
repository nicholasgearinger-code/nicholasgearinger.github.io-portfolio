// Fluid V5 M7.2 bootstrap wrapper — full scenario lab + corrected physical gravity pour.
// Builds from the last full-scenario M7.1.2 bootstrap, swaps in the corrected M7.1.4 gravity module,
// and keeps every existing scenario available. Gravity-pour isolation remains runtime-only.
...
if(!response.ok)throw new Error(`Fluid V5 M7.2: unable to load full-scenario bootstrap (${response.status}).`);
let src=await response.text();

const oldImport="await import('./v5-gravity-pour-m71.js');";
const newImport="await import('./v5-gravity-pour-m714.js');";
if(!src.includes(oldImport))throw new Error('Fluid V5 M7.2: gravity-module import signature changed.');
...
const newActivate=`function activateGravityPour(label='AUTO'){
 ...
 const S=window.__v5GravityPourM71;
 if(!S?.online){
  if(stats)stats.textContent=\`M7.2 \${label}: gravity module unavailable · \${S?.error||'not loaded'}\`;
  return false;
 }
 if(typeof S.activate==='function'){
  try{S.activate(label);return true;}catch(err){console.error('[Fluid V5 M7.2 direct activation]',err);}
 }
 const replay=document.getElementById('v5M71Replay');
 if(!replay)return false;
 replay.click();
...
src=src.replaceAll('M7.1.2','M7.2').replaceAll('7.1.2-m71','7.2-m71');
const blob=URL.createObjectURL(new Blob([src],{type:'text/javascript'}));
try{await import(blob);}finally{URL.revokeObjectURL(blob);}

window.__fluidV5Version='7.2-m71';
window.__fluidV5Build='M7.2 PHYSICAL WATER · DEFINITIVE UI + PERF';
console.info('[Fluid V5 M7.2] full scenario bootstrap online with corrected gravity-pour seeding.');
