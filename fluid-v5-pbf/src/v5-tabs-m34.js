// Fluid V5 M5.6.1 integrated tab shell: M4/M5 physics plus Safari-safe storm/waterfall status.
const srcUrl=new URL('./v5-tabs.js',import.meta.url);
const response=await fetch(srcUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 tabs M5.6.1: unable to load tab shell (${response.status}).`);
let src=await response.text();
const swap=(a,b,label)=>{if(!src.includes(a))throw new Error(`Fluid V5 tabs M5.6.1: ${label} signature changed.`);src=src.replace(a,b)};
...
if(devPanel&&!document.getElementById('v5M4Status')){
 ...
 d.textContent='M5.6.1 suite: Physics/XPBD · Whitewater · Surface 2.0 · split-sum GGX IBL · adaptive workload · Safari-safe micro-rain + continuous waterfall · underwater optics.';
 devPanel.appendChild(d);
}
console.info('[Fluid V5 UI] M5.6.1 integrated lab tabs + weather diagnostics enabled.');
