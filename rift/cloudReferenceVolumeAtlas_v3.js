...
} from "./cloudArchetypes_reference_v3.js";
  ...
  for (const lobe of archetype.lobes) {
    ...
    for (let zi = z0; zi <= z1; zi++) {
      ...
      for (let yi = y0; yi <= y1; yi++) {
        ...
        const topGate = 1 - smooth01((ny - 0.95) / 0.045);
...
export function bakeReferenceCloudAtlasData({
  ...
  for (let i = 0; i < voxelCount; i++) {
    for (let c = 0; c < 4; c++) {
      // Slightly stronger macro contrast keeps small satellite families distinct
      // after trilinear filtering without making the lower deck blocky.
      const density = Math.pow(clamp01(channels[c][i]), 0.94);
...
export function createReferenceCloudAtlas(options = {}) {
  ...
  texture.name = "rift-reference-shaped-cloud-atlas-v33";
