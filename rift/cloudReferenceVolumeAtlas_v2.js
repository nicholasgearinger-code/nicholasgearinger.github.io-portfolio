  ...
  for (const lobe of archetype.lobes) {
    ...
    for (let zi = z0; zi <= z1; zi++) {
      ...
      for (let yi = y0; yi <= y1; yi++) {
        ...
        const topGate = 1 - smooth01((ny - 0.94) / 0.055);
...
export function bakeReferenceCloudAtlasData({
  ...
  for (let i = 0; i < voxelCount; i++) {
    for (let c = 0; c < 4; c++) {
      // v3.1 keeps slightly more contrast in the authored macro shape so the
      // smaller crown lobes survive trilinear sampling on the mobile atlas.
      const density = Math.pow(clamp01(channels[c][i]), 0.90);
...
export function createReferenceCloudAtlas(options = {}) {
  ...
  texture.name = "rift-reference-shaped-cloud-atlas-v31";
