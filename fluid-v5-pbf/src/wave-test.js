// Fluid V5 M6.2 bootstrap. Production V4.4 stays untouched; V5 development systems fail
// independently. M6.2 replaces every airborne recycle/watchdog path with a closed-loop submerged
// reservoir intake feeding a real PBF waterfall inlet, then reconstructs that fluid Houdini-style.

const V5_BUILD='M6.2 RESERVOIR HOUDINI WATERFALL';
const V5_VERSION='6.2.0-m62';
function stampBuild(){document.title=`Fluid V5 · ${V5_BUILD}`;const brand=document.querySelector('.hud.card.title');if(brand&&brand.textContent!=='FLUID V5 · M6.2')brand.textContent='FLUID V5 · M6.2';const load=document.querySelector('#loading h2');if(load&&load.textContent!=='FLUID V5 · M6.2')load.textContent='FLUID V5 · M6.2';window.__fluidV5Version=V5_VERSION;window.__fluidV5Build=V5_BUILD;}
stampBuild();
...
const earlyStats=document.getElementById('v4stats');if(earlyStats)earlyStats.textContent='BUILD: M6.2 · RESERVOIR PUMP · HOUDINI BODY · WHITEWATER · MIST · waiting for V4.4 core…';
...
if(!(await waitForV44()))throw new Error('Fluid V5 M6.2 bootstrap: V4.4 runtime did not become ready within 12 seconds.');stampBuild();
...
// Authoritative M6.2 waterfall stack. There is intentionally NO M5.8/M6.0/M6.1 airborne
// recycler imported here: reservoir pump -> microdrops -> Houdini reconstructed body/whitewater/mist.
try{await import('./v5-waterfall-pump-m62.js');}catch(err){window.__v5WaterfallM62={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.2] reservoir pump rejected.',err);}
try{await import('./v5-microdrops-m59.js');}catch(err){window.__v5MicroDropsM59={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.2] microdroplet surfacing rejected.',err);}
try{await import('./v5-waterfall-houdini-m62.js');}catch(err){window.__v5WaterfallM60={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.2] Houdini reconstructed body/whitewater/mist rejected.',err);}
...
stampBuild();const stats=document.getElementById('v4stats');if(stats&&!stats.textContent.includes('BUILD:'))stats.textContent=`BUILD: M6.2 RESERVOIR + HOUDINI · ${stats.textContent}`;
...
console.info(`[Fluid V5] ${V5_BUILD} / ${V5_VERSION} enabled; no airborne waterfall recycling is loaded.`);
