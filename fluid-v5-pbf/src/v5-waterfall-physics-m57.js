// Fluid V5 M5.7.1 wide physics-based waterfall.
// The waterfall body is real PBF fluid, emitted as a broad continuously extruded curtain using
// the same ballistic-lattice principle as Pour. A wide multi-particle nozzle, per-lane velocity
// variation, gravity and the normal PBF/SSFR/whitewater stack create the falling sheet and breakup.

const sim=window.__sim,ui=window.__ui,state=window.__v5State;
if(!sim?.appendFluid||!state)throw new Error('Fluid V5 M5.7.1 waterfall: PBF runtime unavailable.');
...
function choose(){state.scenario='waterfall-m571';ui.pouring=false;stopWave();save();document.getElementById('reset')?.click();resetEmitter();sync();}

function installButton(){
 const old=document.querySelector('[data-m46="waterfall"]')||document.querySelector('[data-m46="waterfall-m57"]');
 if(!old)return false;
 const b=old.cloneNode(true);b.dataset.m46='waterfall-m571';b.textContent='WATERFALL';
 b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();choose();},{capture:true});
 old.replaceWith(b);return true;
}
installButton();
const buttonTimer=setInterval(()=>{if(!document.querySelector('[data-m46="waterfall-m571"]'))installButton();},550);void buttonTimer;
...
function step(now){
 ...
 if(state.scenario!=='waterfall-m571'||ui.paused||document.hidden||dt<=0||budget()<=0)return;
 ...
 const S=window.__v5WaterfallM57;if(S){S.particlesAdded=added;S.remaining=budget();S.layers=nextLayer;S.width=c.width;S.thickness=c.thick;S.across=c.across;S.impactX=impactPoint(c).x;}
}
requestAnimationFrame(step);

function sync(){
 const b=document.querySelector('[data-m46="waterfall-m571"]');if(b)b.classList.toggle('active',state.scenario==='waterfall-m571');
 const s=document.getElementById('v5WaterfallM57Status');if(s){const c=config();s.textContent=`WIDE REAL PBF CURTAIN · ${c.across}×${c.thick} nozzle lattice · ${(c.width/c.b[2]*100).toFixed(0)}% pool width · +${added.toLocaleString()} particles`;}
}
function mount(){
 ...
 w.innerHTML=`<div style="font:800 9px ui-monospace;color:#9dffc8;letter-spacing:.10em">PHYSICS WATERFALL · M5.7.1</div><div style="font:7.4px/1.45 ui-monospace;color:#86a8b5;margin-top:5px">Broad continuously extruded PBF curtain. The nozzle is now several particle layers thick and spans most of the pool width; small lane-to-lane velocity differences let the real fluid separate into waterfall ribbons under gravity.</div><div class="v5Slider"><label>WATERFALL WIDTH</label><input id="v5WaterfallWidth" type="range" min="0.45" max="0.90" step="0.01"><div id="v5WaterfallWidthVal" class="v5Val"></div></div><div id="v5WaterfallM57Status" style="font:7.5px/1.45 ui-monospace;color:#9fc5d0;margin-top:6px"></div>`;
...
window.__v5WaterfallM57={online:true,backend:'wide-ballistic-pbf-curtain-m571',particlesAdded:0,remaining:budget(),layers:0,width:0,thickness:0,across:0,impactX:0};
console.info('[Fluid V5 M5.7.1] wide physics-based waterfall curtain online.');
