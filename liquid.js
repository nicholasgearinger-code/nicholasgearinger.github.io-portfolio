...
const FLUID_SIM_WIDTH = 128; // cells per side — matches the prototype's own starting point; compute cost scales with cell count, worth raising once this stage is confirmed working live
...
function buildCrystalFluidSimPlane(scene, y, size) {
  ...
  const computeUpdate = Fn(() => {
    ...
    const jitter = fluidSimHash21(vec2(x.mul(0.08), yy.mul(0.08))).sub(0.5).mul(0.4);
    const chop1 = sin(x.mul(0.22).add(yy.mul(0.09)).add(time.mul(1.3)).add(jitter));
    const chop2 = sin(x.mul(0.11).sub(yy.mul(0.18)).sub(time.mul(0.9)).add(jitter.mul(0.7)));
    const chop3 = sin(x.mul(0.35).add(yy.mul(0.31)).add(time.mul(1.7)).add(1.7).add(jitter.mul(1.3)));
