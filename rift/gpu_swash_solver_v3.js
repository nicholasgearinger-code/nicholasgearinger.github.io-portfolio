...
// -----------------------------------------------------------------------------
// Swash v3: true run-up / backwash solve on the existing shoreline strip.
//
// v1/v2 used a real transported foam tracer, but the base wet/dry permit and thin
// film damping were conservative enough that many beaches visually stopped at
// the mean shoreline. v3 keeps the same compact GPU grid and render geometry but
// replaces the compute step with a more permissive finite-depth update:
//
//   • cells can wet up to ~2.6 world units above mean water level;
//   • the offshore boundary is strongly driven by the existing FFT-coupled
//     shallow-water solution;
//   • thin films retain momentum long enough to climb the sampled beach;
//   • gravity/pressure plus a small slope drain produces visible backwash;
//   • foam is an advected tracer, generated at hydraulic bores/wet-dry fronts,
//     carried inland with run-up, and returned seaward with the same velocity.
//
// No procedural shore mask or animated foam texture is used.
// -----------------------------------------------------------------------------
...
function installRunupSolver(handle, sourceShallow, shoreline) {
  ...
  const computeUpdate = Fn(() => {
    ...
    const thin = float(1).sub(smoothstep(float(0.008), float(0.16), h));
    const drag = float(0.30).add(thin.mul(2.65));
    ...
    const sourceH = max(mc.x.add(sourceEta.mul(1.82)), float(0));
    const sourceU = sourceState.y.mul(b.x).add(sourceState.z.mul(b.y)).mul(1.42);
    const sourceV = sourceState.y.mul(b.z).add(sourceState.z.mul(b.w));
    const sourceBreak = clamp(sourceState.w, 0, 1);
    const boundary = float(1).sub(smoothstep(float(0), float(4.8), rf));

    const targetH = min(sourceH.add(sourceBreak.mul(0.08)), float(4.2));
    const targetU = sourceU.add(sourceBreak.mul(0.46));
    h = mix(h, targetH, boundary.mul(0.50));
    uVel = mix(uVel, targetU, boundary.mul(0.42));
    vVel = mix(vVel, sourceV, boundary.mul(0.20));
    ...
    h = h.mul(permit);
    const wet = smoothstep(float(0.0010), float(0.012), h).mul(permit);

    const celerity = sqrt(float(GRAVITY).mul(max(h, float(0.012))));
    const maxSpeed = min(float(4.6), celerity.mul(1.08).add(0.52));
    ...
    const offshoreWet = smoothstep(float(0.003), float(0.030), off.x);
    const landDry = float(1).sub(smoothstep(float(0.0015), float(0.014), land.x));
    const wetFront = wet.mul(offshoreWet).mul(landDry);
    const shallowBand = float(1).sub(smoothstep(float(0.55), float(1.55), h));
    const hydraulicBreak = smoothstep(float(0.58), float(0.96), froude)
      .mul(shallowBand)
      .mul(wet);

    const crossSelector = smoothstep(float(-0.025), float(0.025), uVel);
    const alongSelector = smoothstep(float(-0.025), float(0.025), vVel);
    ...
    foam = mix(foam, foamAverage, min(dt.mul(0.58), float(0.06)));

    const foamDecay = max(float(0), float(1).sub(dt.mul(0.10)));
    foam = clamp(foam.mul(foamDecay), 0, 1);

    const advancing = smoothstep(float(0.05), float(0.72), max(uVel, float(0)));
    const frontFoam = wetFront.mul(float(0.72).add(advancing.mul(0.28)));
    const boreFoam = hydraulicBreak.mul(float(0.48).add(clamp(speed.mul(0.14), 0, 0.34)));
    foam = max(foam, frontFoam);
    foam = max(foam, boreFoam);
    foam = max(foam, sourceBreak.mul(boundary).mul(0.52));

    const filmKeep = smoothstep(float(0.0007), float(0.0045), h);
    h = h.mul(filmKeep);
    uVel = uVel.mul(filmKeep);
    vVel = vVel.mul(filmKeep);
    foam = foam.mul(smoothstep(float(0.0005), float(0.0032), h));
...
function installFoamRendering(handle) {
  ...
  const film = smoothstep(float(0.0008), float(0.022), sampled.x);
  const landDry = float(1).sub(smoothstep(float(0.0012), float(0.012), landSample.x));
  const offshoreWet = smoothstep(float(0.0025), float(0.024), offSample.x);
  const wetFront = film.mul(landDry).mul(offshoreWet);

  const runup = smoothstep(float(0.055), float(0.62), max(sampled.y, float(0)));
  const backwash = smoothstep(float(0.045), float(0.52), max(sampled.y.negate(), float(0)));
  const speed = abs(sampled.y).add(abs(sampled.z).mul(0.28));
  const moving = smoothstep(float(0.035), float(0.48), speed).mul(film);

  const transported = smoothstep(float(0.018), float(0.48), sampled.w).mul(film);
  const leadingWhite = wetFront.mul(float(0.78).add(runup.mul(0.20)));
  const movingWhite = transported.mul(float(0.72).add(moving.mul(0.28)));
  const returningWhite = transported.mul(backwash).mul(0.64);
  ...
  const waterOpacity = film.mul(
    float(0.055)
      .add(smoothstep(float(0.010), float(0.14), sampled.x).mul(0.13))
      .add(aboveMeanSand.mul(0.025)),
  );

  material.colorNode = mix(handle.waterColor, handle.foamColor, foam);
  material.roughnessNode = mix(
    float(0.11).add(handle.storm.mul(0.04)),
    float(0.68),
    foam,
  );
  material.emissiveNode = handle.foamColor.mul(
    foam.mul(float(0.018).add(handle.day.mul(0.026))),
  );
  material.opacityNode = clamp(
    waterOpacity
      .add(foam.mul(float(0.74).add(handle.day.mul(0.22))))
...
export function createGPUSwashSolver(scene, sampleHeight, waterY, sourceShallow, shoreline) {
  ...
  console.info("[gpu-swash] ACTIVE v3: wet/dry run-up + advected foam + backwash");
  return handle;
}
