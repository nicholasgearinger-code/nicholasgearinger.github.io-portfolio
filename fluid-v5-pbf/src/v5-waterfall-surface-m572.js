...
const base={
 ratio:mesh.anisoRatio,stretch:mesh.anisoStretch,lambda:mesh.anisoLambda,minN:mesh.anisoMinNeighbours,
 radius:mesh.anisoRadiusScale,lonely:mesh.anisoLonely,delta:ssfr.narrowDelta,mu:ssfr.narrowMu,
 bilateral:ssfr.bilateralRange,cleanup:ssfr.cleanupRadius,filter:ssfr.filterIterations,
};
let tuned=false;
...
function tuneSurface(){
 if(active()){
  ...
  mesh.anisoMinNeighbours=Math.min(Number.isFinite(base.minN)?base.minN:17,Math.round(12-4*s));
  mesh.anisoRadiusScale=Math.max(base.radius||0,1.95+.22*s);
  mesh.anisoLonely=Math.min(Number.isFinite(base.lonely)?base.lonely:.92,.66-.25*s);
  ...
  mesh.anisoRatio=base.ratio;mesh.anisoStretch=base.stretch;mesh.anisoLambda=base.lambda;
  mesh.anisoMinNeighbours=base.minN;mesh.anisoRadiusScale=base.radius;mesh.anisoLonely=base.lonely;
  ssfr.narrowDelta=base.delta;ssfr.narrowMu=base.mu;ssfr.bilateralRange=base.bilateral;
  ssfr.cleanupRadius=base.cleanup;ssfr.filterIterations=Math.max(ssfr.filterIterations||0,base.filter||0);
  tuned=false;
 }
}
setInterval(tuneSurface,120);tuneSurface();

const uni=dev.createBuffer({label:'fluidV5M572WaterfallVisualUniform',size:96,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});
...
@vertex fn vs(@builtin(vertex_index)vi:u32,@builtin(instance_index)ii:u32)->O{
 ...
 let px=mix(.55,.86,clamp(sp*.22,0.0,1.0))*C.tune.z;let halfW=px*2.0/max(C.screen.x,1.0);let halfL=max(al*.72,px*2.8/max(C.screen.y,1.0));let ndc=cn+normal*q.x*halfW+along*q.y*halfL;
...
@fragment fn fs(v:O)->@location(0)vec4f{
 let r=dot(v.q,v.q);if(r>1){discard;}let core=1.0-smoothstep(.05,1.0,r);let edge=pow(1.0-sqrt(max(0.0,1.0-r)),.42);let fast=clamp(v.speed*.18,0.0,1.0);let col=mix(vec3f(.20,.66,.86),vec3f(.88,.97,1.0),.34+fast*.38+edge*.18);let a=core*(.075+.075*fast)*(1.0-.22*v.age);return vec4f(col,a);
}`;
let pipe=null,bg=null;
try{
 ...
 bg=dev.createBindGroup({layout:pipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:uni}},{binding:1,resource:{buffer:sim.livePos()}},{binding:2,resource:{buffer:sim.liveVel()}}]});
...
ssfr.render=function(...args){
 ...
 const count=Math.min(MAX_PHYS,last-first);UF.set(matMul(proj,view),0);UF[16]=w;UF[17]=h;UF[18]=0;UF[19]=0;UU[20]=first;UU[21]=count;UU[22]=SUBS;UU[23]=last;
 const b=sim.params.box,d=sim.params.spacing;UF[20]=first; // overwritten below via UU alias intentionally
 UF[0+0]=UF[0+0];
 // meta lives in words 20..23; tune follows in a second write because this compact uniform aliases float/u32.
 dev.queue.writeBuffer(uni,0,UF.subarray(0,20));dev.queue.writeBuffer(uni,80,UU.subarray(20,24));
 const tune=new Float32Array([b[1]*.28,d,Math.max(.65,state.waterfallSmooth),Math.max(.2,b[1]*.31)]);dev.queue.writeBuffer(uni,80+16,tune);
