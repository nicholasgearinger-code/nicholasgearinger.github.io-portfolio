// Fluid V8 M8.3.5 — WGSL-safe low-budget continuous faucet + waterfall flow.
// Fixes WebKit rejection of multi-component swizzle assignment while retaining
// the conservative M8.3.4 source budget: at most one source row per rendered frame.

const sim=window.__sim;
const modern=window.__v5M752PhysicalScenes;
const scenes=window.__v5M830Scenes;
if(!sim?.dev||!modern?.online||!scenes?.online) throw new Error('M8.3.5 source flow: runtime unavailable.');
const dev=sim.dev;
...
fn main(@builtin(global_invocation_id) gid:vec3u){
  ...
  if(modeVal==1u){
    ...
    if(tubeMask>0.0){
      let aligned=mix(v.xyz,flowVel,.12*tubeMask)-perp*(.95*dt*tubeMask);
      v=vec4f(aligned,v.w);
    }
  }
  ...
  if(modeVal==2u){
    ...
    if(sheetMask>0.0){
      let alignedXY=mix(v.xy,flowVel.xy,.14*sheetMask);
      let alignedZ=mix(v.z,0.0,.12*sheetMask);
      v=vec4f(alignedXY,alignedZ,v.w);
    }
  }
...
const shaderMod=dev.createShaderModule({code:wgsl,label:'fluidV5M835SourceWGSL'});
if(typeof shaderMod.getCompilationInfo==='function'){
  const info=await shaderMod.getCompilationInfo();
  const errors=(info.messages||[]).filter(m=>m.type==='error');
  if(errors.length) throw new Error('M8.3.5 source WGSL: '+errors.map(m=>`${m.lineNum||'?'}:${m.linePos||'?'} ${m.message}`).join(' | '));
}
const pipe=await dev.createComputePipelineAsync({label:'fluidV5M835Source',layout:'auto',compute:{module:shaderMod,entryPoint:'main'}});
const uni=dev.createBuffer({label:'fluidV5M835SourceUniform',size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});
...
function encodeSource(enc,name){
  ...
  const pass=enc.beginComputePass({label:name==='faucet'?'fluidV5M835Faucet':'fluidV5M835Waterfall'});
...
dev.createCommandEncoder=function(desc){
  ...
  if(inStep){
    ...
    if(name!=='none'){
      try{encodeSource(enc,name)}catch(err){console.error('[M8.3.5 source]',err)}
...
if(note)note.textContent='WGSL-safe low-budget sources · common water';
window.__v5M835SourceFlow={online:true,backend:'wgsl-safe-distance-cadence-m835',gpuPassesAddedWhenActive:1,gpuSubmitsAdded:0,
  get passes(){return passes},get recycled(){return recycled},get emissions(){return emissions},get scene(){return activeSource()}};
window.__fluidV5Version='8.3.5';
window.__fluidV5Build='M8.3.5 WGSL-SAFE CONTINUOUS FAUCET + WATERFALL / M8.2 COMMON WATER / ONE-SUBMIT CORE';
const title=document.querySelector('.hud.card.title');if(title)title.textContent='FLUID V8 · M8.3.5';
document.title='Fluid V8 · M8.3.5 WGSL-safe Sources';
console.info('[Fluid V8 M8.3.5] WGSL-safe low-budget faucet + waterfall online; added submits 0.');