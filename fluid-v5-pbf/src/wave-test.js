// Fluid V5 M6.3 bootstrap. Production V4.4 stays untouched; V5 development systems fail
// independently. M6.3 moves the closed-loop submerged reservoir pump immediately after the PBF
// solver step, giving WebGPU a deterministic solve -> source -> render queue order.

const V5_BUILD='M6.3 POST-STEP RESERVOIR HOUDINI WATERFALL';
const V5_VERSION='6.3.0-m63';
function stampBuild(){document.title=`Fluid V5 · ${V5_BUILD}`;const brand=document.querySelector('.hud.card.title');if(brand&&brand.textContent!=='FLUID V5 · M6.3')brand.textContent='FLUID V5 · M6.3';const load=document.querySelector('#loading h2');if(load&&load.textContent!=='FLUID V5 · M6.3')load.textContent='FLUID V5 · M6.3';window.__fluidV5Version=V5_VERSION;window.__fluidV5Build=V5_BUILD;}
stampBuild();
...
const earlyStats=document.getElementById('v4stats');if(earlyStats)earlyStats.textContent='BUILD: M6.3 · POST-STEP GPU PUMP · RESERVOIR · HOUDINI BODY · waiting for V4.4 core…';
...
if(!(await waitForV44()))throw new Error('Fluid V5 M6.3 bootstrap: V4.4 runtime did not become ready within 12 seconds.');stampBuild();
...
// Authoritative M6.3 waterfall stack. The reservoir source submits immediately after sim.step(),
// never from inside the SSFR renderer. The M6.3 module also exposes an on-screen exact-error badge.
try{await import('./v5-waterfall-pump-m63.js');}catch(err){window.__v5WaterfallM62={...(window.__v5WaterfallM62||{}),online:false,error:String(err?.message||err),backend:'closed-loop-reservoir-pump-m63-post-step'};console.error('[Fluid V5 M6.3] reservoir pump rejected.',err);}
try{await import('./v5-microdrops-m59.js');}catch(err){window.__v5MicroDropsM59={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.3] microdroplet surfacing rejected.',err);}
try{await import('./v5-waterfall-houdini-m62.js');}catch(err){window.__v5WaterfallM60={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.3] Houdini reconstructed body/whitewater/mist rejected.',err);}
...
stampBuild();const stats=document.getElementById('v4stats');if(stats&&!stats.textContent.includes('BUILD:'))stats.textContent=`BUILD: M6.3 RESERVOIR + HOUDINI · ${stats.textContent}`;
...
console.info(`[Fluid V5] ${V5_BUILD} / ${V5_VERSION} enabled; waterfall pump runs after the PBF solve.`);
