// Fluid V5 M3.4.6 tab-shell loader: linear HDR time-of-day UI + smooth submerged flood diagnostics.
const srcUrl=new URL('./v5-tabs.js',import.meta.url);
const response=await fetch(srcUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 tabs M3.4.6: unable to load tab shell (${response.status}).`);
let src=await response.text();
const swap=(a,b,label)=>{if(!src.includes(a))throw new Error(`Fluid V5 tabs M3.4.6: ${label} signature changed.`);src=src.replace(a,b)};
...
"  ['water','WATER','Tune the underlying water material and optical response independently of the scene mood.'],\n  ['lighting','LIGHTING','Choose Day, Sunset or Night. Day and Sunset use your supplied panoramas converted to linear Radiance HDR before cubemap lighting. Night uses a true black HDR environment and six smoothly blended submerged flood fixtures.'],",'tab definitions');
