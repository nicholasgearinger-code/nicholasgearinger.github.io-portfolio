// Fluid V5 M3.1 tab-shell loader: split WATER from LIGHTING and mount the Multi-Light Lab.
const srcUrl=new URL('./v5-tabs.js',import.meta.url);
const response=await fetch(srcUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 tabs M3.1: unable to load tab shell (${response.status}).`);
let src=await response.text();
const swap=(a,b,label)=>{if(!src.includes(a))throw new Error(`Fluid V5 tabs M3.1: ${label} signature changed.`);src=src.replace(a,b)};
...
"  ['water','WATER','Tune the optical material itself: exposure, absorption, thickness, roughness, IOR-related response, continuous-wave testing, and the inherited water rendering controls.'],\n  ['lighting','LIGHTING','Choose Sun, Spot, Point, Underwater, or Skylight rigs. Presets and type-specific controls change real receiver lighting; Sun, Spot, and Point can also drive the atomic caustic projector.'],",'tab definitions');
...
const hudReplacement="const light=window.__v5LightState;hud.textContent=`V5 M3.1 · ${q}${state?.autoQuality?' AUTO':''}\\nFPS ${fps} · SSFR ${scale}% · GPU ${pressure}%\\nPBF ${total.toLocaleString()} · fluid ${fluid.toLocaleString()} · rigid ${rigid.toLocaleString()}\\nsecondary ${m2?.secondaryCapacity?.toLocaleString?.()||'--'} · drained ${m2?.drainedTotal?.toLocaleString?.()||'0'}\\natomic ${atomic.trim()} · light ${(light?.activeType||'sun').toUpperCase()} · UW ${uw.toFixed(2)} m`;";
...
console.info('[Fluid V5 UI] M3.1 WATER + LIGHTING tab split enabled.');
