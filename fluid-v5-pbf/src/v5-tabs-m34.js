// Fluid V5 M5.6.2 integrated tab shell: M4/M5 physics plus ripple-rain / sheet-waterfall status.
const srcUrl=new URL('./v5-tabs.js',import.meta.url);
const response=await fetch(srcUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 tabs M5.6.2: unable to load tab shell (${response.status}).`);
let src=await response.text();
const swap=(a,b,label)=>{if(!src.includes(a))throw new Error(`Fluid V5 tabs M5.6.2: ${label} signature changed.`);src=src.replace(a,b)};
...
if(devPanel&&!document.getElementById('v5M4Status')){
 ...
 d.textContent='M5.6.2 suite: Physics/XPBD · Surface 2.0 · split-sum GGX IBL · propagating rain ring packets · accelerating layered waterfall sheet · independent impact mist.';
 devPanel.appendChild(d);
}
console.info('[Fluid V5 UI] M5.6.2 integrated lab tabs + ripple/weather diagnostics enabled.');
