...
function getCloudOcclusionFactor(handle, cameraPos, targetPos) {
  ...
  let occlusion = 0;
  // Per "flickering lighting when storm is active" — real, confirmed
  // cause: this used to be a hard binary cutoff (if alignment > 0.9975),
  // zero smoothing. As a cloud drifts (or the camera turns even
  // slightly), its alignment value crosses back and forth across that
  // exact line continuously — each crossing SNAPS occlusion instantly
  // between 0 and cloud.baseOpacity, which is what showed up as a real,
  // measured frame-to-frame brightness alternation (confirmed by
  // sampling actual video luminance, not assumed). Storms make this far
  // more likely to be visible specifically because more/denser clouds
  // mean a much higher chance one is hovering near that threshold at
  // any given moment. Replaced with a smoothstep ramp across a small
  // window BELOW the old cutoff — a cloud approaching/leaving alignment
  // now fades in/out continuously, the actual fix (removing the hard
  // edge), not just a tighter/looser threshold that would still have one.
  const ALIGN_EDGE = 0.9975;
  const ALIGN_SOFTNESS = 0.0025;
  for (const cloud of handle.clouds) {
    ...
    const t = Math.max(0, Math.min(1, (alignment - (ALIGN_EDGE - ALIGN_SOFTNESS)) / ALIGN_SOFTNESS));
    const smoothT = t * t * (3 - 2 * t); // smoothstep — continuous, not a snap
    if (smoothT > 0) occlusion = Math.max(occlusion, cloud.baseOpacity * smoothT);
  }
