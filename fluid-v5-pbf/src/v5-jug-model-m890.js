// Fluid V8 M8.9.0 — robust high-poly jug replacement via standards-compliant glTF decoding.
// Three.js is used only as a GLB decoder (including Draco/Meshopt when needed). Decoded
// triangles are copied into the existing native WebGPU vessel render pass; Three.js never renders.
...
function fitToPitcher(src){
  ...
  const bodyX=xz.length?xz.reduce((a,p)=>a+p[0],0)/xz.length:(b.lo[0]+b.hi[0])*.5;
  const bodyZ=xz.length?xz.reduce((a,p)=>a+p[1],0)/xz.length:(b.lo[2]+b.hi[2])*.5;
  let spoutX=bodyX+1,spoutZ=bodyZ,best=-1;
  for(let i=0;i<src.length;i+=6){const t=(src[i+1]-b.lo[1])/h;if(t<.72)continue;const dx=src[i]-bodyX,dz=src[i+2]-bodyZ,r=dx*dx+dz*dz;if(r>best){best=r;spoutX=src[i];spoutZ=src[i+2];}}
  const autoYaw=Math.atan2(spoutZ-bodyZ,spoutX-bodyX);
  const yaw=autoYaw+(Number(q.get('jugyaw'))||0)*Math.PI/180,c=Math.cos(yaw),si=Math.sin(yaw);
  const baseY=-.255,yOff=baseY-b.lo[1]*s;
  const out=new Float32Array(src.length);
  for(let i=0;i<src.length;i+=6){
    const x=(src[i]-bodyX)*s,z=(src[i+2]-bodyZ)*s,y=src[i+1]*s+yOff;
    const nx=src[i+3],ny=src[i+4],nz=src[i+5];
    const rx=c*x+si*z,rz=-si*x+c*z,rnx=c*nx+si*nz,rnz=-si*nx+c*nz,l=Math.hypot(rnx,ny,rnz)||1;
...
async function decodeGLB(){
  ...
  const draco=new DRACOLoader();draco.setDecoderPath(DRACO_DECODER);draco.setDecoderConfig({type:'wasm'});loader.setDRACOLoader(draco);loader.setMeshoptDecoder(MeshoptDecoder);
  ...
  gltf.scene.traverse(obj=>{
    ...
    const g=obj.geometry,pos=g.attributes.position,nor=g.attributes.normal,idx=g.index;normalMat.getNormalMatrix(obj.matrixWorld);meshes++;
    ...
    if(nor){for(let k=0;k<count;k++)emit(idx?idx.getX(k):k);triangles+=Math.floor(count/3);}
    else{
      for(let k=0;k+2<count;k+=3){
        const ia=idx?idx.getX(k):k,ib=idx?idx.getX(k+1):k+1,ic=idx?idx.getX(k+2):k+2;
        const a=new THREE.Vector3().fromBufferAttribute(pos,ia).applyMatrix4(obj.matrixWorld),b=new THREE.Vector3().fromBufferAttribute(pos,ib).applyMatrix4(obj.matrixWorld),c=new THREE.Vector3().fromBufferAttribute(pos,ic).applyMatrix4(obj.matrixWorld);
        const fn=new THREE.Vector3().subVectors(b,a).cross(new THREE.Vector3().subVectors(c,a)).normalize();
        for(const v of [a,b,c])vals.push(v.x,v.y,v.z,fn.x,fn.y,fn.z);triangles++;
      }
    }
  });
  draco.dispose();
...
async function build(){
  ...
  const vb=dev.createBuffer({label:'m890JugVB',size:g.data.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});queue.writeBuffer(vb,0,g.data);
  const code=`
...
@fragment fn fs(i:O,@builtin(front_facing)front:bool)->@location(0)vec4f{
 var n=normalize(i.n);if(!front){n=-n}let v=normalize(u.eye.xyz-i.w);let l=normalize(u.light.xyz);
 let ndv=clamp(abs(dot(n,v)),0.0,1.0);let fres=pow(1.0-ndv,2.35);let spec=pow(max(dot(reflect(-l,n),v),0.0),120.0);
 let edge=pow(1.0-ndv,5.0);let col=vec3f(.76,.93,.985)*(.74+.15*max(n.y,0.0))+vec3f(1.0)*(spec*.90+edge*.34);
 return vec4f(col,clamp(.075+.38*fres+.10*edge,.065,.56))
  ...
  if(typeof sm.getCompilationInfo==='function'){
    ...
    if(errors.length)throw new Error('M8.9.0 jug WGSL: '+errors.map(m=>m.message).join(' | '));
  }
  const pipe=dev.createRenderPipeline({label:'m890DecodedHighPolyJug',layout:'auto',vertex:{module:sm,entryPoint:'vs',buffers:[{arrayStride:24,attributes:[{shaderLocation:0,offset:0,format:'float32x3'},{shaderLocation:1,offset:12,format:'float32x3'}]}]},fragment:{module:sm,entryPoint:'fs',targets:[{format,blend:{color:{srcFactor:'src-alpha',dstFactor:'one-minus-src-alpha',operation:'add'},alpha:{srcFactor:'one',dstFactor:'one-minus-src-alpha',operation:'add'}}}]},primitive:{topology:'triangle-list',cullMode:'none'}});
  ...
  state.prepare=({vp,eye})=>{F.fill(0);F.set(vp,0);F[16]=eye[0];F[17]=eye[1];F[18]=eye[2];F[19]=1;F[20]=-.35;F[21]=.82;F[22]=.45;F[24]=pitcher.cx;F[25]=pitcher.cy;F[26]=pitcher.cz;F[27]=pitcher.angle;queue.writeBuffer(ub,0,F);};
...
function installHud(S){
  ...
  let d=document.getElementById('m890Status');if(!d){d=document.createElement('div');d.id='m890Status';d.style.cssText='margin-top:6px;padding-top:6px;border-top:1px solid rgba(112,225,235,.20);color:#91dfe7';hud.appendChild(d);}
  const sync=()=>{if(!S?.ready){d.textContent='new jug load error · '+(S?.error||'unknown');return;}const f=S.g.fittedExt.map(v=>v.toFixed(2)).join('×');d.textContent=`NEW JUG ACTIVE · ${S.meshes} meshes · ${S.triangles.toLocaleString()} tris · fit ${f} m · draws ${S.draws.toLocaleString()}`;};sync();setInterval(sync,500);
}

let S=window[KEY];
if(phase==='pre'){
  try{if(!S?.ready){S=await build();window[KEY]=S;}window.__v5PitcherVisualHook=S;}
  catch(e){console.error('[M8.9.0] jug decode failed',e);S={ready:false,error:String(e?.message||e)};window[KEY]=S;delete window.__v5PitcherVisualHook;}
}else{
  if(S?.ready)window.__v5PitcherVisualHook=S;else delete window.__v5PitcherVisualHook;installHud(S);
}

window.__fluidV5Version='8.9.0';
window.__fluidV5Build='M8.9.0 STANDARDS GLB JUG REPLACEMENT / DRACO + MESHOPT DECODE / M8.8.8 CLEAN CATCH / M8.8.1 FLUID PHYSICS';
