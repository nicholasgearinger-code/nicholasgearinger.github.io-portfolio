...
function createEruptionFountain(coneH) {
  const group = new THREE.Group();
  const streaks = [];
  for (let i = 0; i < 22; i++) { // was 14 — bigger, denser burst
    const length = 2.5 + Math.random() * 5; // was 2-5.5 — some genuinely large shards now, not just fine spray
    ...
    streaks.push({
      ...
      speed: 6 + Math.random() * 5, phase: Math.random(),
    });
  }
...
function createEmberLandmark(colorHex) {
  ...
  pool.rotation.y = yaw;
  group.add(pool);
  const craterLight = new THREE.PointLight(0xff6a2a, 1.2, 30);
  ...
  const smoke = createCraterSmoke(0, 5);
