// Fluid V5 M7.1 bootstrap — isolated physical gravity-pour benchmark.
// When ?m71=pour is present, the scenario is selected before legacy weather/waterfall systems and
// those source/detail modules are not imported. The benchmark therefore contains only primary PBF
// liquid, shared physics/density systems, sampled collision geometry and the normal SSFR surface.

const qp=new URLSearchParams(location.search);
const ISOLATED=qp.get('m71')==='pour';
const V5_BUILD='M7.1 PHYSICAL WATER · ISOLATED GRAVITY POUR';
const V5_VERSION='7.1-m71';
function stampBuild(){
 document.title=`Fluid V5 · ${V5_BUILD}`;
 const brand=document.querySelector('.hud.card.title');if(brand&&brand.textContent!=='FLUID V5 · M7.1')brand.textContent='FLUID V5 · M7.1';
 const load=document.querySelector('#loading h2');if(load&&load.textContent!=='FLUID V5 · M7.1')load.textContent='FLUID V5 · M7.1';
...
const earlyStats=document.getElementById('v4stats');if(earlyStats)earlyStats.textContent='BUILD: M7.1 · ISOLATED PRIMARY PBF POUR · waiting for V4.4 core…';
...
if(!(await waitForV44()))throw new Error('Fluid V5 M7.1 bootstrap: V4.4 runtime did not become ready within 12 seconds.');
stampBuild();
...
// In isolated mode none of the legacy source systems are even imported.
if(!ISOLATED){
...
stampBuild();
const stats=document.getElementById('v4stats');if(stats&&!stats.textContent.includes('BUILD:'))stats.textContent=`BUILD: M7.1 ISOLATED PRIMARY PBF POUR · ${stats.textContent}`;
