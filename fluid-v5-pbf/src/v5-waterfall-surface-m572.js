// Fluid V5 M5.8.3 slab-aware continuous fixed-mass waterfall.
// Reuse the validated M5.8.1 tagged SSFR exclusion + thin-sheet renderer, but disable its legacy
// recycle plane. M5.8.3 owns the only recycle pass and derives the true free-surface height from
// the same lattice calculation used by v5-pool-slab.js.

const baseUrl='https://raw.githubusercontent.com/nicholasgearinger-code/nicholasgearinger.github.io-portfolio/5ed701b737ef39093c403e9933746a19685b0a28/fluid-v5-pbf/src/v5-waterfall-surface-m572.js';
const response=await fetch(baseUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 M5.8.3 base surface unavailable (${response.status}).`);
let baseSrc=await response.text();
// Disable the obsolete M5.8.1 recycle pass. Its b[1]*.28 surface assumption is wrong for the
// full-floor V5 pool slab and can trap/recycle particles at the wrong height.
const legacy=' if(enc&&sim.n>0){';
if(!baseSrc.includes(legacy))throw new Error('Fluid V5 M5.8.3: legacy recycle signature changed.');
baseSrc=baseSrc.replace(legacy,' if(false&&enc&&sim.n>0){');
const blobUrl=URL.createObjectURL(new Blob([baseSrc],{type:'text/javascript'}));
try{await import(blobUrl);}finally{URL.revokeObjectURL(blobUrl);}

const sim=window.__sim,ssfr=window.__ssfr,state=window.__v5State;
if(!sim?.dev||!ssfr?.dev||!state)throw new Error('Fluid V5 M5.8.3 recycle: runtime unavailable.');
const dev=sim.dev,TAG=window.__v5WaterfallTag||0x5746;
const active=()=>state.scenario==='waterfall-m58';
const clamp=(v,a,b)=>Math.min(b,Math.max(a,v));

function slabSurfaceY(){
 const b=sim.params.box,d=sim.params.spacing,margin=d;
 const nx=Math.max(1,Math.floor((b[0]-2*margin)/d));
 const nz=Math.max(1,Math.floor((b[2]-2*margin)/d));
 const baseFluid=Math.max(1,Number(sim.scene?.nFluid)||Math.max(1,sim.n-(sim.nBodyParts||0)-(window.__v5WaterfallM57?.target||0)));
 const layers=Math.max(1,Math.ceil(baseFluid/(nx*nz)));
 return clamp(margin+layers*d,d*2,b[1]-d*2);
}

// 64-byte uniform: three vec4f blocks + one vec4u block.
// geo0 = topY, surfaceY, nozzleX, centreZ
// geo1 = width, spacing, vx, vy
// geo2 = impactX, contactY, reboundY, overshootX
// meta = n, tag, frame, enabled
const uni=dev.createBuffer({label:'fluidV5M583RecycleUniform',size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});
const UF=new Float32Array(16),UU=new Uint32Array(UF.buffer);
const wgsl=`
struct U{geo0:vec4f,geo1:vec4f,geo2:vec4f,meta:vec4u}
@group(0)@binding(0)var<uniform>C:U;
@group(0)@binding(1)var<storage,read_write>P:array<vec4f>;
@group(0)@binding(2)var<storage,read_write>V:array<vec4f>;
@group(0)@binding(3)var<storage,read_write>phase:array<vec4u>;
fn hash1(x0:u32)->f32{var x=x0;x^=x>>16u;x*=0x7feb352du;x^=x>>15u;x*=0x846ca68bu;x^=x>>16u;return f32(x)/4294967295.0;}
@compute @workgroup_size(256)fn main(@builtin(global_invocation_id)gid:vec3u){
 let i=gid.x;if(i>=C.meta.x||C.meta.w==0u){return;}
 var ph=phase[i];if(ph.w!=C.meta.y){return;}
 let p=P[i].xyz;let v=V[i].xyz;
 // The contact band is tied to the actual slab surface. Overshoot/rebound are safety nets for
 // particles deflected by the density solve before their centres cross the nominal contact plane.
 let contact=p.y<=C.geo2.y;
 let rebound=p.y<=C.geo2.z&&v.y>-.12;
 let overshoot=p.x>=C.geo2.w;
 if(!(contact||rebound||overshoot)){return;}
 let base=select(i+1u,ph.y,ph.y!=0u);
 let cycle=ph.z+1u;
 let s=base^(cycle*2246822519u)^(C.meta.z*3266489917u);
 let h0=hash1(s+17u);let h1=hash1(s+101u);let h2=hash1(s+313u);let h3=hash1(s+911u);
 let z=C.geo0.w+(h0-.5)*C.geo1.x*.96;
 let x=C.geo0.z+(h1-.5)*C.geo1.y*.18;
 // Spread recycled parcels over ~3 particle spacings so the closed loop behaves like a steady
 // inlet buffer rather than a synchronized pulse.
 let y=C.geo0.x-C.geo1.y*(.10+2.90*h2);
 let vz=(h3-.5)*.032;
 P[i]=vec4f(x,y,z,1.0);
 V[i]=vec4f(C.geo1.z+(h2-.5)*.016,C.geo1.w-h1*.028,vz,0.0);
 phase[i]=vec4u(ph.x,ph.y,cycle,ph.w);
}`;
const mod=dev.createShaderModule({code:wgsl,label:'fluidV5M583RecycleWGSL'});
if(typeof mod.getCompilationInfo==='function'){
 const info=await mod.getCompilationInfo();
 const errors=(info.messages||[]).filter(m=>m.type==='error');
 if(errors.length)throw new Error('Fluid V5 M5.8.3 recycle WGSL: '+errors.map(m=>`${m.lineNum||'?'}:${m.linePos||'?'} ${m.message}`).join(' | '));
}
const pipe=await dev.createComputePipelineAsync({label:'fluidV5M583SlabAwareRecycle',layout:'auto',compute:{module:mod,entryPoint:'main'}});
let frame=0;
const baseRender=ssfr.render;
ssfr.render=function(...args){
 const enc=args[0];
 if(enc&&sim.n>0&&active()){
  const b=sim.params.box,d=sim.params.spacing;
  const flow=clamp(Number(state.waterfallFlow)||1,.45,1.55);
  const surface=slabSurfaceY();
  const topY=clamp(surface+Math.min(.79,b[1]*.315),surface+d*6,b[1]-d*2.5);
  const nozzleX=Math.max(d*1.65,b[0]*.038);
  const vx=.23+.085*flow,vy=-.055-.030*flow,g=Math.max(1,Number(sim.params.gravity)||9.81);
  const h=Math.max(0,topY-surface);
  const t=(vy+Math.sqrt(Math.max(0,vy*vy+2*g*h)))/g;
  const impactX=nozzleX+vx*t;
  const width=Math.min(b[2]*.92,Math.max(d*14,b[2]*clamp(Number(state.waterfallWidth)||.78,.48,.92)));
  UF[0]=topY;UF[1]=surface;UF[2]=nozzleX;UF[3]=b[2]*.50;
  UF[4]=width;UF[5]=d;UF[6]=vx;UF[7]=vy;
  UF[8]=impactX;
  // Recycle after a genuine surface interaction band rather than waiting below the surface.
  UF[9]=surface+d*1.85;
  UF[10]=surface+d*2.75;
  UF[11]=impactX+d*.45;
  UU[12]=sim.n;UU[13]=TAG;UU[14]=(frame++>>>0);UU[15]=1;
  dev.queue.writeBuffer(uni,0,UF);
  const phase=sim.liveBody?.()||sim.buf[sim.parity===0?'bodyA':'bodyB'];
  const bg=dev.createBindGroup({layout:pipe.getBindGroupLayout(0),entries:[
   {binding:0,resource:{buffer:uni}},
   {binding:1,resource:{buffer:sim.livePos()}},
   {binding:2,resource:{buffer:sim.liveVel()}},
   {binding:3,resource:{buffer:phase}},
  ]});
  const cp=enc.beginComputePass();cp.setPipeline(pipe);cp.setBindGroup(0,bg);cp.dispatchWorkgroups(Math.max(1,Math.ceil(sim.n/256)));cp.end();
  const R=window.__v5WaterfallRecycleM583;if(R){R.surfaceY=surface;R.impactX=impactX;R.frame=frame;}
 }
 return baseRender.apply(this,args);
};

window.__v5WaterfallRecycleM583={online:true,backend:'slab-aware-single-recycle-m583',tag:TAG,surfaceY:0,impactX:0,frame:0};
window.__v5WaterfallRecycleM582=window.__v5WaterfallRecycleM583;
if(window.__v5WaterfallSurfaceM572)window.__v5WaterfallSurfaceM572.backend='fixed-mass-sheet-plus-slab-recycle-m583';
setTimeout(()=>{
 const brand=document.querySelector('.hud.card.title');if(brand)brand.textContent='FLUID V5 · M5.8.3';
 document.title='Fluid V5 · M5.8.3 SLAB-AWARE CONTINUOUS WATERFALL';
 window.__fluidV5Version='5.3.8.3-m583';
},1200);
console.info('[Fluid V5 M5.8.3] single slab-aware continuous waterfall recycle online.');
