// Fluid V5 M7.1.3 gravity-pour benchmark.
// Primary PBF water is physically relocated into an elevated trough at rest. A sampled solid gate
// initially contains the reservoir. Releasing the gate removes only collision support; no launch
// velocity, trajectory guide, analytic curtain, or waterfall-specific force is applied.

const sim=window.__sim,ui=window.__ui,state=window.__v5State,ssfr=window.__ssfr,cam=window.__cam;
if(!sim?.dev||!ui||!state||!ssfr)throw new Error('Fluid V5 M7.1.3 gravity pour: runtime unavailable.');
const dev=sim.dev;
...
if(Number(state.gravityPourRev||0)<5){
 state.gravityPourHeight=.84;
 state.gravityPourWidth=.985;
 state.gravityPourVolume=.58;
 state.gravityPourAutoRelease=true;
 state.gravityPourRev=5;
}
if(!Number.isFinite(Number(state.gravityPourHeight)))state.gravityPourHeight=.84;
if(!Number.isFinite(Number(state.gravityPourWidth)))state.gravityPourWidth=.985;
if(!Number.isFinite(Number(state.gravityPourVolume)))state.gravityPourVolume=.58;
if(typeof state.gravityPourAutoRelease!=='boolean')state.gravityPourAutoRelease=true;
state.gravityPourHeight=clamp(Number(state.gravityPourHeight),.68,.90);
state.gravityPourWidth=clamp(Number(state.gravityPourWidth),.72,.998);
state.gravityPourVolume=clamp(Number(state.gravityPourVolume),.30,.68);
save();

let baselineBoundaryPts=[];
let gateClosed=true;
let releaseTimer=0,rebuildTimer=0,releasedAt=0;
...
function geom(){
 ...
 const floorY=b[1]*clamp(Number(state.gravityPourHeight)||.84,.68,.90);
 const lipX=b[0]*.72;
 const backX=Math.max(d*1.15,b[0]*.035);
 const width=b[2]*clamp(Number(state.gravityPourWidth)||.985,.72,.998);
 const z0=(b[2]-width)*.5,z1=z0+width;
 const wallTop=Math.min(b[1]-d*.70,floorY+Math.max(d*10,b[1]*.16));
 const upperNx=Math.max(3,Math.floor((lipX-backX-d*2.2)/d));
 const upperNz=Math.max(3,Math.floor((width-d*2.3)/d));
 ...
 const upperWanted=Math.round(nFluid*clamp(Number(state.gravityPourVolume)||.58,.30,.68));
 const maxUpperLayers=Math.max(4,Math.floor((wallTop-floorY-d*.65)/d));
 ...
 const lowerSurface=Math.min(floorY-d*8,d*(.78+lowerLayers));
...
function buildBoundary(includeGate){
 ...
 const base=baselineBoundaryPts?.length?baselineBoundaryPts:(sim.scene?.boundary?.pts||[]);
 for(let i=0;i<base.length;i+=3)add(base[i],base[i+1],base[i+2]);
 const step=d*.88;
 // Elevated shelf: two sampled layers, terminating sharply at lipX.
 for(let x=g.backX;x<=g.lipX+step*.12;x+=step)
  for(let z=g.z0;z<=g.z1+step*.12;z+=step){add(x,g.floorY,z);add(x,g.floorY-d*.50,z);}
 // Rear and side walls form a real reservoir. The front remains open after gate removal.
  ...
  for(let dz=-1;dz<=1;dz++){const zz=e+dz;if(zz<0||zz>=dim[2])continue;
   for(let dy=-1;dy<=1;dy++){const yy=c+dy;if(yy<0||yy>=dim[1])continue;
    for(let dx=-1;dx<=1;dx++){const xx=a+dx;if(xx<0||xx>=dim[0])continue;
     ...
     for(const j of arr){const rx=pts[i*3]-pts[j*3],ry=pts[i*3+1]-pts[j*3+1],rz=pts[i*3+2]-pts[j*3+2],r2=rx*rx+ry*ry+rz*rz;if(r2>=h2)continue;const t=h2-r2;sum+=t*t*t;}
    }
   }
  }
...
function installBoundary(includeGate){
 ...
 sim.buf.bpos=dev.createBuffer({label:'fluidV5M713BoundaryPos',size:Math.max(16,n*16),usage:ST});
 sim.buf.bpsi=dev.createBuffer({label:'fluidV5M713BoundaryPsi',size:Math.max(16,n*4),usage:ST});
 sim.buf.bcellStart=dev.createBuffer({label:'fluidV5M713BoundaryCellStart',size:Math.max(16,(sim.nCells+2)*4),usage:ST});
...
const seedUni=dev.createBuffer({label:'fluidV5M713SeedUniform',size:112,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});
const SF=new Float32Array(28),SU=new Uint32Array(SF.buffer);
const seedCounter=dev.createBuffer({label:'fluidV5M713SeedCounter',size:16,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});
const seedWGSL=`
...
@compute @workgroup_size(256)fn main(@builtin(global_invocation_id)gid:vec3<u32>){
 ...
 if(rank<C.meta.y){
  ...
  p=vec3<f32>(C.g0.x+d*(1.12+f32(ix)),C.g0.y+d*(.84+f32(iy)),C.g0.z+d*(1.12+f32(iz)));
 }else{
...
const seedMod=dev.createShaderModule({code:seedWGSL,label:'fluidV5M713SeedWGSL'});
if(typeof seedMod.getCompilationInfo==='function'){
 ...
 if(errors.length)throw new Error('Fluid V5 M7.1.3 seed WGSL: '+errors.map(m=>`${m.lineNum||'?'}:${m.linePos||'?'} ${m.message}`).join(' | '));
}
const seedPipe=await dev.createComputePipelineAsync({label:'fluidV5M713Seed',layout:'auto',compute:{module:seedMod,entryPoint:'main'}});
...
function seedWater(){
 ...
 dev.queue.writeBuffer(seedUni,0,SF);const enc=dev.createCommandEncoder({label:'fluidV5M713SeedEncoder'});enc.clearBuffer(seedCounter);
...
function tune(on){
 if(on){
  ...
  sim.params.sCorrK=Math.min(Number(sim.params.sCorrK)||.032,.040);
  sim.params.surfaceTensionK=Math.min(Number(sim.params.surfaceTensionK)||.050,.060);
...
function setError(err,where='setup'){
 const msg=String(err?.message||err);const S=window.__v5GravityPourM71;
 if(S){S.ready=false;S.error=`${where}: ${msg}`;}
 const stats=document.getElementById('v4stats');if(stats)stats.textContent=`M7.1.3 ${where.toUpperCase()} ERROR · ${msg}`;
 console.error(`[Fluid V5 M7.1.3 ${where}]`,err);
}

function setupFromFreshBase(){
 baselineBoundaryPts=[...(sim.scene?.boundary?.pts||[])];
 gateClosed=true;releasedAt=0;
 installBoundary(true);seedWater();tune(true);scheduleAutoRelease();
 const S=window.__v5GravityPourM71;if(S){S.active=true;S.ready=true;S.gateClosed=true;S.error='';S.activationSerial=activationSerial;}
 sync();
}

const baseReset=sim.reset.bind(sim);
function activateNow(reason='UI'){
 activationSerial++;
 clearTimeout(releaseTimer);clearTimeout(rebuildTimer);
 state.scenario=ACTIVE;ui.pouring=false;stopWave();save();
 try{
  // IMPORTANT: bypass the DOM reset button and the M7 wrapper itself. Start from the exact base reset
  // captured when this module loaded, then build the reservoir once, explicitly, in this call.
  const out=baseReset(sim.params);
  state.scenario=ACTIVE;ui.pouring=false;save();
  setupFromFreshBase();
  if(cam){const g=geom();cam.target=[g.b[0]*.50,g.floorY*.58,g.b[2]*.50];cam.el=Math.max(.16,Math.min(.34,Number(cam.el)||.25));cam.dist=Math.max(Number(cam.dist)||0,3.7);}
  const S=window.__v5GravityPourM71;if(S)S.lastActivation=reason;
  return out??true;
 }catch(err){setError(err,`activate ${reason}`);return false;}
}

sim.reset=function(params){
 const out=baseReset(params);
 if(active()){
  try{setupFromFreshBase();}catch(err){setError(err,'reset setup');}
 }else{tune(false);}
 return out;
};
...
function release(){
 if(!active()||!gateClosed)return false;
 try{gateClosed=false;clearTimeout(releaseTimer);releaseTimer=0;installBoundary(false);sim.primeGrid();releasedAt=performance.now();const S=window.__v5GravityPourM71;if(S)S.releasedAt=releasedAt;sync();return true;}
 catch(err){setError(err,'gate release');return false;}
}
function scheduleAutoRelease(){clearTimeout(releaseTimer);if(active()&&state.gravityPourAutoRelease&&gateClosed)releaseTimer=setTimeout(()=>release(),1800);}
function scheduleRebuild(){clearTimeout(rebuildTimer);rebuildTimer=setTimeout(()=>{if(active())activateNow('CONTROL REBUILD');},420);}

function sync(){
 const g=geom(),S=window.__v5GravityPourM71,drift=massReference?((Number(sim.n)||0)-massReference):0;
 const st=document.getElementById('v5GravityPourM71Status');if(st)st.textContent=`${g.upperN.toLocaleString()} elevated + ${g.lowerN.toLocaleString()} pool · ${g.headLayers} head layers · ${g.drop.toFixed(2)} m drop · gate ${gateClosed?'CLOSED':'OPEN'}${S?.ready?' · READY':''}`;
 ...
 if(stats&&active()){
  const base=stats.textContent.split('\n').filter(x=>!x.startsWith('M7.1')).slice(0,6);
  base.push(`M7.1.3 POUR ${S?.ready?'READY':'WAIT'} · ${actualSubsteps||sim.params.substeps} sub · ${actualIterations||sim.params.iterations} iter`);
  base.push(`M7.1.3 MASS Δ${drift} · ${g.upperN.toLocaleString()} elevated · gate ${gateClosed?'CLOSED':'OPEN'}`);
  stats.textContent=base.join('\n');
 }
 const mode=document.getElementById('v4mode');if(mode&&active())mode.textContent='M7.1.3 GRAVITY POUR';
...
function mount(){
 ...
 d.innerHTML=`<div style="font:800 9px ui-monospace;color:#9dffc8;letter-spacing:.10em">PHYSICAL GRAVITY POUR · M7.1.3</div><div style="font:7.4px/1.45 ui-monospace;color:#86a8b5;margin-top:5px">The same primary PBF water is moved into a tall full-width reservoir at ZERO velocity. The gate is collision geometry. When it opens, pressure + gravity alone create the pour.</div><div class="v5Slider" style="margin-top:8px"><label>POUR HEIGHT</label><input id="v5M71Height" type="range" min="0.68" max="0.90" step="0.005"><div class="v5Val" id="v5M71HeightVal"></div></div><div class="v5Slider"><label>OPENING WIDTH</label><input id="v5M71Width" type="range" min="0.72" max="0.998" step="0.005"><div class="v5Val" id="v5M71WidthVal"></div></div><div class="v5Slider"><label>ELEVATED WATER</label><input id="v5M71Volume" type="range" min="0.30" max="0.68" step="0.02"><div class="v5Val" id="v5M71VolumeVal"></div></div><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:7px"><button id="v5M71Activate" class="v5Btn" type="button">START GRAVITY POUR</button><button id="v5M71Release" class="v5Btn" type="button">RELEASE GATE</button><button id="v5M71Replay" class="v5Btn" type="button">RESET POUR</button><button id="v5M71Auto" class="v5Btn" type="button"></button></div><div id="v5GravityPourM71Status" style="font:7.5px/1.45 ui-monospace;color:#9fc5d0;margin-top:6px"></div>`;h.appendChild(d);
 ...
 height.oninput=e=>{state.gravityPourHeight=clamp(Number(e.target.value),.68,.90);save();sync();scheduleRebuild();};
 width.oninput=e=>{state.gravityPourWidth=clamp(Number(e.target.value),.72,.998);save();sync();scheduleRebuild();};
 volume.oninput=e=>{state.gravityPourVolume=clamp(Number(e.target.value),.30,.68);save();sync();scheduleRebuild();};
 d.querySelector('#v5M71Activate').onclick=e=>{e.stopPropagation();activateNow('PANEL START');};
 d.querySelector('#v5M71Release').onclick=e=>{e.stopPropagation();release();};
 d.querySelector('#v5M71Replay').onclick=e=>{e.stopPropagation();activateNow('PANEL RESET');};
 d.querySelector('#v5M71Auto').onclick=e=>{e.stopPropagation();state.gravityPourAutoRelease=!state.gravityPourAutoRelease;save();if(state.gravityPourAutoRelease)scheduleAutoRelease();else{clearTimeout(releaseTimer);releaseTimer=0;}sync();};
 d.onpointerdown=e=>e.stopPropagation();sync();
}

window.__v5GravityPourM71={online:true,backend:'direct-activation-gated-primary-pbf-m713',active:false,ready:false,error:'',gateClosed:true,boundarySamples:0,upperParticles:0,lowerParticles:0,dropHeight:0,width:0,headLayers:0,releasedAt:0,zeroLaunchVelocity:true,physicalGate:true,fixedPrimaryMass:true,legacySources:false,actualSubsteps:0,actualIterations:0,massReference:0,massCurrent:0,massDrift:0,massConserved:true,blockedAppendParticles:0,activationSerial:0,lastActivation:'',activate:activateNow,release,reset:()=>activateNow('PUBLIC RESET')};
setInterval(()=>{mount();sync();},220);mount();sync();

if(new URLSearchParams(location.search).get('m71')==='pour'){
 setTimeout(()=>activateNow('QUERY AUTO'),420);
 setTimeout(()=>{const S=window.__v5GravityPourM71;if(!S?.ready||!(S.upperParticles>0))activateNow('QUERY WATCHDOG');},1400);
}
console.info('[Fluid V5 M7.1.3] direct-activation physical gravity-pour benchmark online.');
