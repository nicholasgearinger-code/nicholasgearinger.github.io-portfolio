// Fluid V5 M6.6.1 bootstrap. Production V4.4 stays untouched; V5 development systems fail
// independently. M6.6.1 decouples coarse PBF impact physics from the fine visible waterfall surface:
// solve -> low-rate hidden PBF impact stream -> coherent guide -> corrected resampled curtain/mist -> render.

const V5_BUILD='M6.6.1 HYBRID FLIP-STYLE RESAMPLED WATERFALL';
const V5_VERSION='6.6.1-m661';
function stampBuild(){document.title=`Fluid V5 · ${V5_BUILD}`;const brand=document.querySelector('.hud.card.title');if(brand&&brand.textContent!=='FLUID V5 · M6.6.1')brand.textContent='FLUID V5 · M6.6.1';const load=document.querySelector('#loading h2');if(load&&load.textContent!=='FLUID V5 · M6.6.1')load.textContent='FLUID V5 · M6.6.1';window.__fluidV5Version=V5_VERSION;window.__fluidV5Build=V5_BUILD;}
stampBuild();
...
const earlyStats=document.getElementById('v4stats');if(earlyStats)earlyStats.textContent='BUILD: M6.6.1 · LOW-RATE PBF IMPACT · CORRECTED RESAMPLED CURTAIN · waiting for V4.4 core…';
...
if(!(await waitForV44()))throw new Error('Fluid V5 M6.6.1 bootstrap: V4.4 runtime did not become ready within 12 seconds.');stampBuild();
...
// Authoritative M6.6.1 waterfall stack. The hidden PBF stream transfers real momentum/displacement;
// the visible waterfall is a fine resampled ballistic surface, not the centimeter-scale particles.
try{await import('./v5-waterfall-pump-m66.js');}catch(err){window.__v5WaterfallM62={...(window.__v5WaterfallM62||{}),online:false,error:String(err?.message||err),backend:'low-rate-impact-primary-m66'};console.error('[Fluid V5 M6.6.1] low-rate PBF impact source rejected.',err);}
try{await import('./v5-waterfall-guide-m64.js');}catch(err){window.__v5WaterfallGuideM64={online:false,error:String(err?.message||err),backend:'post-pbf-ballistic-curtain-guide-m64'};console.error('[Fluid V5 M6.6.1] coherent waterfall guide rejected.',err);}
try{await import('./v5-waterfall-whitewater-gate-m65.js');}catch(err){window.__v5WaterfallWhitewaterM65={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.6.1] waterfall whitewater ownership failed.',err);}
try{await import('./v5-microdrops-m59.js');}catch(err){window.__v5MicroDropsM59={online:false,error:String(err?.message||err)};console.error('[Fluid V5 M6.6.1] general splash microdroplet surfacing rejected.',err);}
try{await import('./v5-waterfall-houdini-m661.js');}catch(err){window.__v5WaterfallM60={...(window.__v5WaterfallM60||{}),online:false,error:String(err?.message||err),backend:'resampled-ballistic-curtain-m661-safari'};console.error('[Fluid V5 M6.6.1] corrected resampled waterfall surface/mist rejected.',err);}
...
stampBuild();const stats=document.getElementById('v4stats');if(stats&&!stats.textContent.includes('BUILD:'))stats.textContent=`BUILD: M6.6.1 LOW-RATE PBF + RESAMPLED CURTAIN · ${stats.textContent}`;
...
console.info(`[Fluid V5] ${V5_BUILD} / ${V5_VERSION} enabled; waterfall rendering is decoupled from solver particle size.`);
