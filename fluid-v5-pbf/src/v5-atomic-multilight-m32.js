// Fluid V5 M3.2: distinctive multi-light atomic caustics.
// Keeps the validated full-surface particle + atomic<u32> -> r32uint path. Sun, Spot and Point
// provide different photon fields; preset caustic gain and emitter color now survive to the floor.

const srcUrl=new URL('./v5-atomic-fullsurface.js',import.meta.url);
const response=await fetch(srcUrl,{cache:'no-store'});
if(!response.ok)throw new Error(`Fluid V5 M3.2: unable to load atomic source (${response.status}).`);
...
const lightPatch=` let lightType=U.dims.w;
 ...
 if(lightType==1u||lightType==2u){
   ...
   incident=toP/dist;
   let rangeFade=1.0-smoothstep(U.pos.w*0.68,U.pos.w,dist);
   lightGain*=rangeFade/(1.0+0.26*dist*dist);
   if(lightType==1u){
...
swap(' let w=U.values.x*(0.17+0.83*incidence)*(0.80+0.72*slope)*0.23;',' let w=lightGain*(0.12+0.88*incidence)*(0.72+0.92*slope)*0.25;','photon weight');
...
const encodePatch=` const L=window.__v5LightLab?.getPackedState?.()||window.__v5LightState||{};
 ...
 projectF[12]=lc[0]??1;projectF[13]=lc[1]??1;projectF[14]=lc[2]??1;
 projectF[15]=Math.max(0,(L.intensity??1)*(L.causticGain??1));
...
// High-pass photon density: only local focusing survives, never the broad direct beam.
...
// Expand overlay uniform to carry selected emitter color.
swap("label:'fluidV5FullSurfaceAtomicOverlayUniform',size:16","label:'fluidV5FullSurfaceAtomicOverlayUniform',size:32",'overlay uniform size');
swap('const overlayF=new Float32Array(4);','const overlayF=new Float32Array(8);','overlay uniform view');
swap('struct O { strength:f32, debug:f32, gain:f32, pad:f32 }','struct O { strength:f32, debug:f32, gain:f32, pad:f32, tint:vec4f }','overlay tint struct');
...
const curvePatch=`fn light(v:f32)->vec3f{
 let focused=max(v-0.0012,0.0);
 let c=1.0-exp(-focused*3.65*U.gain);
 let h=smoothstep(0.0018,0.54,c);
 let base=vec3f(h,h*.97,h*.88);
 return base*mix(vec3f(1.0),U.tint.rgb,vec3f(.72));
}`;
...
const receiverPatch=` let uv=(p.xz-C.boxMin.xz)/max(C.boxMax.xz-C.boxMin.xz,vec2f(1.0e-4));
 ...
 let alpha=clamp(peak*U.strength*.18,0.0,.32);
 return vec4f(c*(.56+U.strength*.46),alpha);`;
swap(receiverNeedle,receiverPatch,'receiver focus');

const overlayEncodeNeedle=` const raw=window.__v5DebugMode==='atomic';
...
const overlayEncodePatch=` const raw=window.__v5DebugMode==='atomic';
 if(!raw&&state.projected<=.002)return;
 const L=window.__v5LightLab?.getPackedState?.()||window.__v5LightState||{};const lc=L.color||[1,1,1];
 overlayF[0]=state.projected;overlayF[1]=raw?1:0;overlayF[2]=1;overlayF[3]=0;
 overlayF[4]=lc[0]??1;overlayF[5]=lc[1]??1;overlayF[6]=lc[2]??1;overlayF[7]=1;
 dev.queue.writeBuffer(overlayUni,0,overlayF);`;
swap(overlayEncodeNeedle,overlayEncodePatch,'overlay light tint upload');

src=src.replaceAll("backend:'particle-full'","backend:'particle-multilight-m32'");
src=src.replaceAll('full-surface particle caustics online','distinct multi-light full-surface caustics online');
const blobUrl=URL.createObjectURL(new Blob([src],{type:'text/javascript'}));
try{await import(blobUrl)}finally{URL.revokeObjectURL(blobUrl)}
if(window.__v5ProjectedCaustics)window.__v5ProjectedCaustics.backend='particle-multilight-m32';
if(window.__v5AtomicStatus)window.__v5AtomicStatus.backend='particle-multilight-m32';
console.info('[Fluid V5 M3.2] distinctive multi-light atomic caustics enabled.');
