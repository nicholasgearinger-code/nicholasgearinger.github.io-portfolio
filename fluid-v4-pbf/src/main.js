...
const QUALITY = {
  low: {
    label: 'LOW', short: 'LOW', particles: '18400', spacing: '0.031',
    ssfrscale: '0.38', substeps: '2', iters: '3', ssfriters: '2', ssfrthickblur: '16',
    note: 'Low · ~18K particles · 38% surface · fastest mobile mode.'
  },
  medium: {
    label: 'MEDIUM', short: 'MED', particles: '30000', spacing: '0.026',
    ssfrscale: '0.52', substeps: '2', iters: '4', ssfriters: '3', ssfrthickblur: '20',
    note: 'Medium · ~30K particles · 52% surface · recommended mobile balance.'
  },
  high: {
    label: 'HIGH', short: 'HIGH', particles: '48600', spacing: '0.022',
    ssfrscale: '0.68', substeps: '3', iters: '5', ssfriters: '4', ssfrthickblur: '24',
    note: 'High · ~49K particles · 68% surface · smoother water, heavier GPU load.'
  }
};
const qualityName = QUALITY[q.get('quality')] ? q.get('quality') : 'medium';
const quality = QUALITY[qualityName];

// The physical pool is identical at every quality. Particle spacing/count change together so
// selecting Low does not mean draining the pool; it means representing the same volume more cheaply.
const defaults = {
  preset: 'small', view: 'ssfr', quality: qualityName,
  box: '1.9 1.2 1.25', bodies: '1', body: 'sphere:0.55:0.82', bodysize: '0.09',
  grab: '1', grabstrength: '23', force: '1', fradius: '0.15', fstrength: '36', flimit: '6',
  ...
  ssfrradius: '0.72', ssfrdelta: '8.6', ssfrmu: '1.04',
  ssfrthickr: '1.75', ssfrthick: '0.55', ssfrthickblur: quality.ssfrthickblur,
  ior: '1.333', absorption: '0.425', transmit: '0.34902 0.705882 0.894118', roughness: '0.048',
  camera: '-0.72 0.54 3.45 0.95 0.48 0.625', floorplane: '1', cubemap: '', v4ui: '1'
};
...
const CAMERA = { az: -0.72, el: 0.54, dist: 3.45, target: [0.95, 0.48, 0.625] };
...
for (const b of document.querySelectorAll('[data-quality]')) {
  ...
  b.onclick = () => {
    const next = b.dataset.quality;
    if (!QUALITY[next] || next === qualityName) {
      settingsPanel.classList.add('hidden');
      return;
    }
    // Start clean so old per-quality query values cannot leak into the next preset.
    location.search = '?' + new URLSearchParams({ quality: next }).toString();
  };
}
...
document.addEventListener('pointerdown', () => settingsPanel.classList.add('hidden'));
...
function waterTap(ray, strength = 2.8) {
  ...
  sim.applyRayImpulse(ray.origin, ray.dir, imp, ui.forceRadius || 0.15, Math.max(ui.forceLimit || 6, 4));
}
function waterDrag(a, b, dx, dy) {
  const gain = 48;
  const imp = [
    (b.dir[0] - a.dir[0]) * gain,
    b.dir[1] * 0.38 + (dy > 0 ? -0.35 : 0),
    (b.dir[2] - a.dir[2]) * gain
  ];
  sim.applyRayImpulse(b.origin, b.dir, imp, (ui.forceRadius || 0.15) * 1.15, Math.max(ui.forceLimit || 6, 5));
}
...
function hudLoop() {
  ...
  statsEl.textContent = `${quality.short} · ${fluid.toLocaleString()} · ${scale ? Math.round(scale * 100) + '% surf' : 'PBF'}`;
