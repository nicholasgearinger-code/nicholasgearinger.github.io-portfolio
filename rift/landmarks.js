...
const LANDMARK_POSITION = { x: 0, z: -30 }; // moved again from (30, -30) per explicit "stretch the coastline, extends far in the distance" follow-up — X centered at 0 gives a full symmetric ±120 units of clearance along that axis (was ~90 at x=30), which is what an elongated coastline actually needs room for. Z clearance stays 90 (unchanged, at the -Z direction), matching the existing beach/hill radius there. This exact coordinate is duplicated as hardcoded literals in terrain.js (crystal's island shaping) and decorations.js (the onIsland check) since those files don't import from here — both were updated to match in the same round this moved.
